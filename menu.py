...
def search_notes(notes, keyword = None, status = None):
    if not keyword and not status:
        print("\n\033[31mПараметры поиска не введены.\033[0m")
    # Поиск только по ключевому слову.
    elif keyword != None and not status:
        j = 0  # Счетчик заметок.
        for i in range(len(notes)):
            cur_values = list()  # Временный список значения словаря.
            cur_values.append(notes[i].get('username'))
            cur_values.append(notes[i].get('title'))
            cur_values.append(notes[i].get('content'))
            for k in range(len(cur_values)):  # Цикл проверки
                if keyword in cur_values[k]:  # наличия ключевого слова во временном списке.
                    created_date = notes[i]['created_date'].strftime("%d %B")
                    issue_date = notes[i]['issue_date'].strftime("%d %B")
                    j += 1
                    if j == 1:
                    ...
                    print(f"\t\033[3;37mПользователь:\033[0m {notes[i]['username']}"
                          ...
                          f"\n\t\033[3;37mДата дедлайна:\033[0m {issue_date}")
        if j == 0:
            print(f"\n\033[31mСлова '{keyword}' в заметках не существует.\033[0m")
        ...
        for i in range(len(notes)):               # Цикл проверки
            if status == notes[i].get('status'):  # наличия статуса в словаре.
                ...
                print(f"\t\033[3;37mПользователь:\033[0m {notes[i]['username']}"
                      ...
                      f"\n\t\033[3;37mДата дедлайна:\033[0m {issue_date}")
        if j == 0:
            print(f"\n\033[31mСтатуса '{status}' в заметках не существует.\033[0m")
            ...
            for k in range(len(cur_values)):                                       # Цикл проверки
                if keyword in cur_values[k] and status == notes[i].get('status'):  # наличия ключевого слова во временном списке и статуса в словаре.
                    ...
                    print(f"\t\033[3;37mПользователь:\033[0m {notes[i]['username']}"
                          ...
                          f"\n\t\033[3;37mДата дедлайна:\033[0m {issue_date}")
        if j == 0:
            print(f"\n\033[31mНе найдены заметки со словом '{keyword}' и статусом '{status}'.\033[0m")
...
while True:
    choice = input("\n\033[35mМеню действий:\033[0m\n\033[33m0.\033[0m Выйти"
                   "\n\033[33m1.\033[0m Добавить заметку\n\033[33m2.\033[0m Обновить заметку"
                   "\n\033[33m3.\033[0m Удалить заметку\n\033[33m4.\033[0m Найти заметки"
                   "\n\033[33m5.\033[0m Вывести список заметок\n\t\033[36mВведите цифру: \033[0m")
    if choice == "5":
        if not notes:  # Проверка на наличие заметок.
            print("\033[31m\n\tПока заметок нет.\033[0m")
            continue
        count_notes = int(input("\nСколько заметок разместить на странице? "))
        count_page = int(len(notes) / count_notes)
        for num_page in range(count_page if (len(notes) % count_notes) == 0 else (count_page + 1)): # Формирование страницы указанного кол-ва заметок
                display_notes(notes, num_page)  # Вывод списка заметок.
    elif choice == "4":
        if not notes:  # Проверка на наличие заметок.
            print("\033[31m\n\tПока заметок нет.\033[0m")
            continue
        print("\nОставьте пункт пустым, если поиск будет выполняться не по его значению.")
        keyword = input("\033[3;36mВведите ключевое слово: \033[0m")
        status = input("\033[3;36mВведите статус: \033[0m")
        # Поиск заметок
        if keyword == '' and status == '':  # без аргументов,
            search_notes(notes)
        elif keyword == '':  # только по статусу,
            search_notes(notes, status = status)
        elif status == '':  # только по ключевому слову,
            search_notes(notes, keyword = keyword)
        else:  # по статусу и ключевому слову.
